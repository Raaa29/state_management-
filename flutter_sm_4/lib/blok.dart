...
import 'package:http/http.dart'
    as http; // Import library untuk membuat permintaan HTTP
...
class UniversityEvent {
  ...
  UniversityEvent(
      this.country); // Konstruktor untuk event, menerima negara sebagai parameter
}

// State untuk menampung daftar universitas.
class UniversityState {
  final List<Universitas>
      universities; // List untuk menyimpan objek Universitas

  UniversityState(
      this.universities); // Konstruktor state, menerima daftar universitas sebagai parameter
}
...
class UniversityBloc extends Bloc<UniversityEvent, UniversityState> {
  UniversityBloc()
      : super(
            UniversityState([])); // Inisialisasi Bloc dengan state awal kosong

  @override
  Stream<UniversityState> mapEventToState(UniversityEvent event) async* {
    yield UniversityState(await _fetchUniversitasList(event
        .country)); // Menghasilkan state baru dengan daftar universitas yang diperbarui
  }
  ...
  Future<List<Universitas>> _fetchUniversitasList(String country) async {
    final response = await http.get(Uri.parse(
        'http://universities.hipolabs.com/search?country=$country')); // Melakukan permintaan HTTP untuk mendapatkan data universitas berdasarkan negara

    if (response.statusCode == 200) {
      // Jika permintaan berhasil
      final List<dynamic> data =
          json.decode(response.body); // Mendekode respon JSON
      return data
          .map((json) => Universitas.fromJson(json))
          .toList(); // Mengonversi data JSON menjadi objek Universitas dan mengembalikan daftar universitas
    } else {
      throw Exception(
          'Failed to fetch universities'); // Jika permintaan gagal, lemparkan pengecualian
    }
  }
}
...
class Universitas {
  ...
  Universitas(
      {required this.nama,
      required this.situs}); // Konstruktor dengan parameter wajib

  // Konstruktor factory untuk membuat objek Universitas dari JSON
  factory Universitas.fromJson(Map<String, dynamic> json) {
    return Universitas(
      nama: json['name'] ??
          'Nama Tidak Tersedia', // Ambil nama universitas, jika tidak tersedia, gunakan nilai default
      situs: json['web_pages'] != null && json['web_pages'].isNotEmpty
          ? json['web_pages']
              [0] // Ambil URL situs web universitas pertama jika tersedia
        ...
        if (state.universities.isEmpty) {
          ...
          return ListView.builder(
            itemCount: state.universities
                .length, // Jumlah item dalam daftar adalah panjang daftar universitas
            itemBuilder: (context, index) {
              final universitas = state.universities[
                  index]; // Ambil objek universitas pada indeks tertentu
              return ListTile(
                title: Text(universitas.nama), // Tampilkan nama universitas
                subtitle: Text(
                    universitas.situs), // Tampilkan URL situs web universitas
                onTap: () {
                  launch(universitas
                      .situs); // Buka situs web universitas saat item di tap
                },
              );
            },
          );
    ...
    return MaterialApp(
      home: BlocProvider(
        create: (context) => UniversityBloc()
          ..add(UniversityEvent(
              'Indonesia')), // Inisialisasi Bloc dan kirim event untuk memperbarui daftar universitas
          ...
          BlocBuilder<UniversityBloc, UniversityState>(
            builder: (context, state) {
              return DropdownButton<String>(
                value:
                    selectedCountry, // Nilai dropdown adalah negara yang dipilih
                onChanged: (String? newValue) {
                  if (newValue != null) {
                    ...
                    context.read<UniversityBloc>().add(UniversityEvent(
                        newValue)); // Kirim event untuk memperbarui daftar universitas
                  }
                },
                items: <String>[
                  'Indonesia',
